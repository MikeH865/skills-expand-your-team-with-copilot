  ...
  function displayFilteredActivities() {
    ...
    Object.entries(allActivities).forEach(([name, details]) => {
      ...
      if (currentDifficulty === "All") {
        // "All Levels" shows only activities with no difficulty specified (suitable for all students)
        if (details.difficulty) {
          return;
        }
      } else if (currentDifficulty) {
        // Specific difficulty level: show only activities tagged with that difficulty
  ...
  async function shareActivity(name, details) {
    ...
    if (navigator.share) {
      ...
      try {
        ...
        showMessage(`"${name}" details copied to clipboard! Share it with your friends.`, "success");
      } catch (error) {
        console.error("Clipboard copy failed:", error);
        showMessage("Unable to share. Please copy the page link manually.", "error");
      }
    }
  }
